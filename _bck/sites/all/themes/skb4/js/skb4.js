(function ($) {
    ...
    attach: function (context, settings) {  		
      ...
      if ((window.innerWidth<='768')){
        ...
        $("#block-views-news-block-1 .view-content, #block-views-news-block-2 .view-content").mCustomScrollbar({
          axis:"x",
          // setTop: 0
          // scrollButtons:{enable:true},
        ...
        $("#block-skb4-helper-skb4-helper-catalog .category.with-subcategories, #block-skb4-helper-skb4-helper-catalog-else .category.with-subcategories, #block-system-main .category.with-subcategories").each(function () {
          let cat = $(this);
      ...
      $(".taxonomy-term.vocabulary-product-category .cats > .item-list").mCustomScrollbar({
        axis:"y",
        // setTop: 0
        scrollButtons:{enable:true},
        theme:"light-thick",
      ...
      $(".taxonomy-term.vocabulary-product-category .cats .toggle-list").click(function () {
        let but = $(this);
        if ($(but).hasClass('open')) {
          $(but).removeClass('open');
          $(".taxonomy-term.vocabulary-product-category .cats > .item-list").slideDown();
          $(".taxonomy-term.vocabulary-product-category .cats").css('height', '340px');
          $("#category-main .taxonomy-term.vocabulary-product-category .cats h3").css('margin-bottom', '20px');
        } else {
          $(but).addClass('open');
          $(".taxonomy-term.vocabulary-product-category .cats").css('height', 'auto');
          $(".taxonomy-term.vocabulary-product-category .cats > .item-list").slideUp(400, function () {
            $("#category-main .taxonomy-term.vocabulary-product-category .cats h3").css('margin-bottom', '0');
          });
      ...
      if ($('.view-id-references').length) {
        ...
        $('.view-id-references').once(function () {
          ...
          $(".view-id-references").before("<div class='jcarousel-pagination-n'></div>");

          $(".jcarousel-pagination-n").append("<div class='section'><a href='#' class='jcarousel-control-prev-n' data-jcarouselcontrol='true'></a><a href='#' class='jcarousel-control-next-n' data-jcarouselcontrol='true'></a></div>");
            
          $('.jcarousel-control-prev-n')
            .jcarouselControl({
              target: '-=1'
          });

          $('.jcarousel-control-next-n')
            .jcarouselControl({
              target: '+=1'
          });
      ...
      if ($('.views-field-field-news-gallery').length) {
        ...
        $('.views-field-field-news-gallery').once(function () {
          ...
          $(".views-field-field-news-gallery").before("<div class='jcarousel-pagination-n'></div>");
      ...
      var $page = $('html, body');
      $('.node-brand.node-view-mode-full a[href*="#"], .category-navigation a[href*="#"]').click(function(event) {
      ...
      if ($('#header').length) {
        let sh = $('#header').stickMe({ topOffset:180, backScrollOffset: 210 });
      }

      $("body.node-type-product #main-wrapper #product-main .node-product .product-content-wrapper .field-name-body table, .node-brand .body-wrapper .field-name-body table").each(function () {
        $(this).parent('div').addClass('with-table');
      });

      $('.node-brand, #block-skb4-helper-skb4-helper-current-offer', context).once('current-offer-wrapper', function () {
      ...
      $('#block-skb4-helper-skb4-helper-current-offer', context).once('content', function () {
  ...
  Drupal.behaviors.skb4_filemanaged = {
    attach: function (context, settings) {
      $('.form-managed-file', context).once('form-managed-file-init', function () {
        ...
        inp.change(function(){
            ...
            if( ! file_name.length )
                return;
            btn.text( file_name );
            $("input[id^='edit-submitted-attach-upload-button']", wrapper1).mousedown();
            /*if( lbl.is( ":visible" ) ){
                lbl.text( file_name );
            }else
                btn.text( file_name );*/
      ...
      $(".form-managed-file input[id^='edit-submitted-attach-upload-button']").css({'display':'none'});
